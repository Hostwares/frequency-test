 ...
 * Mirrors base44/shared/fundedNetworkLimits.ts for the frontend runtime.
 *
 ...
 */
export const FUNDED_NETWORK_LIMITS_BY_PLAN = {
  founding_supporter_beta: 1,
  founding_premium_beta: 3,
  founding_champion_beta: 6,
};

export const TIER_UPGRADE_LABELS = {
  founding_supporter_beta: 'Founding Premium Beta',
  founding_premium_beta: 'Founding Patron Beta',
};

export function getMaxFundedNetworksForPlan(planCode, fallback = 3) {
...
/**
 * Per-tier maximum DISTINCT supported artists across all funded playlists combined.
 *   Beta Supporter → 25, Founding Premium Beta → 50, Founding Patron Beta → 75
 */
export const MAX_DISTINCT_ARTISTS_BY_PLAN = {
  founding_supporter_beta: 25,
  founding_premium_beta: 50,
  founding_champion_beta: 75,
};

export function getMaxDistinctArtistsForPlan(planCode, fallback = 25) {
...
/**
 * Per-tier maximum SONGS per single funded playlist.
 *   Beta Supporter → 25, Founding Premium Beta → 50, Founding Patron Beta → 75
 */
export const MAX_SONGS_PER_FUNDED_PLAYLIST_BY_PLAN = {
  founding_supporter_beta: 25,
  founding_premium_beta: 50,
  founding_champion_beta: 75,
};

export function getMaxSongsPerFundedPlaylistForPlan(planCode, fallback = 100) {
 ...
 */
export const AI_ASSISTANCE_FEATURES_BY_TIER = {
...
export const AI_ASSISTANCE_LABELS = {
...
export function getAIAssistanceFeatures(tier) {
  return AI_ASSISTANCE_FEATURES_BY_TIER[tier] || AI_ASSISTANCE_FEATURES_BY_TIER.standard;
}

export function canAccessAIFeature(tier, feature) {
 ...
 *
 *   standard      → basic counts only (playlist stats, referral count/points)
 *   advanced      → + trends, charts, and breakdowns (play trends, genre/mood, referral growth, leaderboard)
 *   advanced_plus → + deep engagement and conversion data (per-song engagement, network graph, conversion funnel, revenue impact)
 */
export const ANALYTICS_FEATURES_BY_TIER = {
  standard: [
...
export const ANALYTICS_FEATURE_LABELS = {
...
export function getAnalyticsFeatures(tier) {
  return ANALYTICS_FEATURES_BY_TIER[tier] || ANALYTICS_FEATURES_BY_TIER.standard;
}

export function canAccessAnalyticsFeature(tier, feature) {
  return getAnalyticsFeatures(tier).includes(feature);
}

export function getAnalyticsTierLabel(tier) {
 ...
 */
export const SOCIAL_AMPLIFIER_POSTS_BY_PLAN = {
  founding_supporter_beta: 0,
  founding_premium_beta: 2,
  founding_champion_beta: 3,
};

export function getSocialAmplifierPostsForPlan(planCode) {
...
export function canAccessSocialAmplifier(planCode) {
 ...
 */
export const SIGNAL_POINT_MULTIPLIER_BY_PLAN = {
  founding_supporter_beta: 1.0,
  founding_premium_beta: 1.5,
  founding_champion_beta: 2.0,
};

export function getSignalPointMultiplier(planCode, fallback = 1.0) {
